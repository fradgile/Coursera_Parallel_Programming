...
public class DownloadUtils {
	...
	/**
	 * Make an Intent which will start a service if provided as a parameter to
	 * startService().
	 * 
	 * @param context
	 *            The context of the calling component
	 * @param service
	 *            The class of the service to be started. (For example,
	 *            ThreadPoolDownloadService.class)
	 * @param handler
	 *            The handler that the service should use to return a result.
	 * @param uri
	 *            The web URL that the service should download
	 * 
	 ...
	 * 
	 *            Active Object Pattern when it is passed a message, which
	 *            serves as the Active Object, and acts depending on what the
	 *            message contains.
	 * 
	 *            The handler *must* be wrapped in a Messenger to allow it to
	 *            operate across process boundaries.
	 */
	public static Intent makeMessengerIntent(Context context, Class<?> service,
			Handler handler, String uri) {
	...
	/**
	 * Use the provided Messenger to send a Message to a Handler in another
	 * process.
	 * 
	 * The provided string, outputPath, should be put into a Bundle and included
	...
	/**
	 * Download a file to the Android file system, then respond with the file
	 * location using the provided Messenger. Hint: use the other methods in
	 * this class
	 */
	public static void downloadAndRespond(Context context, Uri uri,
			Messenger messenger) {
		sendPath(DownloadUtils.downloadFile(context, uri), messenger);
	}

	/**
	 * The resource that we write to the file system in offline mode. Note that
	 * this must be the same image that the testing project expects. (found in
	 * res/drawable-nodpi and Options.java)
	 */
	...
	/**
	 * Download the file located at the provided internet url using the URL
	 * class, store it on the android file system using openFileOutput(), and
	...
	public static String downloadFile(Context context, Uri uri) {
		...
		try {
			...
			if (DOWNLOAD_OFFLINE) {
				...
				FileOutputStream out = context.openFileOutput(OFFLINE_FILENAME,
						0);
	 ...
	 * @throws IOException
	 */
	static private File getTemporaryFile(final Context context, final String url)
			throws IOException {
		...
		// files!
		// return
		// context.getFileStreamPath(Base64.encodeToString(url.getBytes(),
		// Base64.NO_WRAP)
